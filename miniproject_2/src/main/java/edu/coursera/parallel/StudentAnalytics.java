...
public final class StudentAnalytics {
    ...
    public double averageAgeOfEnrolledStudentsParallelStream(
        ...
        List<Student> activeStudents= students.parallelStream().filter(Student::checkIsCurrent).collect(Collectors.toList());
        double ageSum= activeStudents.parallelStream().mapToDouble(Student::getAge).sum();
        return ageSum / (double) activeStudents.size();
    }
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(
            final Student[] studentArray) {
        List<Student> students= Arrays.asList(studentArray);
        List<Student> inactiveStudents= students.parallelStream().filter(student -> !student.checkIsCurrent()).collect(Collectors.toList());
        Map<String, Integer> nameCounts = inactiveStudents.parallelStream().collect(Collectors.toMap(Student::getFirstName, s -> 1, Integer::sum));
        Optional<Map.Entry<String, Integer>> mostCommonEntry = nameCounts.entrySet().parallelStream()
                .max(Map.Entry.comparingByValue());
        String mostCommon= null;
        int mostCommonCount= 1;
        if (mostCommonEntry.isPresent()) {
            mostCommon = mostCommonEntry.get().getKey();
            mostCommonCount = mostCommonEntry.get().getValue();
        }
        return mostCommon;
    }
    ...
    public int countNumberOfFailedStudentsOlderThan20ParallelStream(
            final Student[] studentArray) {
        List<Student> s= Arrays.asList(studentArray);
        List<Student> studentWAgeCriteria= s.parallelStream().filter(student -> (!student.checkIsCurrent() && student.getAge() > 20 && student.getGrade() < 65)).collect(Collectors.toList());
        return studentWAgeCriteria.size();
    }
}
