    ...
    public static void runParallelFuzzyBarrier(final int iterations,
        ...
        for (int i = 0; i < tasks; i++) {
            ...
            threads[i] = new Thread(() -> {
                double[] thread1 = myVal;
                double[] thread2 = myNew;
                ...
                for (int iter = 0; iter < iterations; iter++) {
                    ...
                    for (int a = left; a <= right; a++) {
                        thread2[a] = (thread1[a - 1]
                                + thread1[a + 1]) / 2.0;
                    }
                    int currentPhase = phaser[j].arrive();
                    if(j-1>= 0) phaser[j-1].awaitAdvance(currentPhase);
                    if(j+1< tasks) phaser[j+1].awaitAdvance(currentPhase);
                    double[] temp = thread2;
                    thread2 = thread1;
                    thread1 = temp;
                }
